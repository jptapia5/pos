...
class productos(QMainWindow):
    def __init__(self):
        ...
        self.statusBar().showMessage(
            "Muestra el listado de los productos registrado en el sistema")
    ...
    def contruirTabla(self):
        ...
        self.listaProductos.verticalHeader().setVisible(
        ...
        self.listaProductos.horizontalHeader().setHighlightSections(False)
        ...
        columnas = ('CODIGO', 'DESCRIPCION', 'STOCK', 'STOCK MIN', 'STOCK MÁX',
                    'PRECIO COSTO', 'PRECIO VENTA', 'CATEGORIA', 'PROVEEDOR')
    ...
    def dobleClick(self):
        ...
        # la fila que modificará, solo una a la vez
        fila = self.listaProductos.currentRow()
        # la columna que modificará, solo una a la vez
        columna = self.listaProductos.currentColumn()
        # el id de la fila que modificará
        id = self.listaProductos.item(fila, 0).text()
        # print(id)
    ...
    def listarTodo(self):
        ...
        for consulta in consultas:
            ...
            sql = "SELECT nombreCategoria FROM categoria_producto WHERE idCategoriaProducto = '" + \
                str(categoria) + "'"
            cursor = conn.cursor()
            cursor.execute(sql)
            ...
            if (colorStock <= colorStockCritico):  # AMARILLO SI ALCANZÓ O BAJO EL STOCK CRITICÓ
                for col in range(9):
                    self.listaProductos.item(row, col).setBackground(
    ...
    def buscar(self):
        ...
        if busqueda == "":
            QMessageBox.warning(
                self, 'Información', "No ha aplicado ningun criterio de búsqueda", QMessageBox.Ok)
        else:
            if self.filtroDescripcion.isChecked():
                ...
                cursor.execute(sql)
                self.habilitarBotones()
                self.listaProductos.setEditTriggers(
                ...
                if not consultas:
                    QMessageBox.information(
                        self, 'Información', "No se encontraron resultados", QMessageBox.Ok)
                else:
                    for consulta in consultas:
                        self.listaProductos.insertRow(row)
                        codigo = QTableWidgetItem(str(consulta[0]))
                        ...
                        self.listaProductos.setItem(row, 7, categoria)
                        self.listaProductos.setItem(row, 8, proveedor)
                        # AMARILLO SI ALCANZÓ O BAJO EL STOCK CRITICÓ
                        if (colorStock <= colorStockCritico):
                            for col in range(9):
                                self.listaProductos.item(row, col).setBackground(
                                    QtGui.QColor(255, 255, 0))
                        # ROJO SI le digo que si es CERO
                        if (colorStock < 2):
                            for col in range(9):
                                self.listaProductos.item(row, col).setBackground(
                                    QtGui.QColor(255, 0, 0))
                        # VERDE si el stock es igual o mayor al maximo stock
                        if (colorStock > colorStockMaximo):
                            for col in range(9):
                                self.listaProductos.item(row, col).setBackground(
                                    QtGui.QColor(0, 255, 0))
                        row = row + 1
                ...
                if busqueda.isdigit():
                    ...
                    if not consultas:
                        ...
                        for consulta in consultas:
                            ...
                            stockCritico = QTableWidgetItem(str(consulta[3]))
                            stockMaximo = QTableWidgetItem(str(consulta[4]))
                            precioCosto = QTableWidgetItem(
                                "$" + str(consulta[5]))
                            precioVenta = QTableWidgetItem(
                                "$" + str(consulta[6]))
                            categoria = (consulta[7])
                            # define el color a destacar
                            colorStock = consulta[2]
    ...
    def modificar(self):
        respuesta = QMessageBox.warning(self, 'Advertencia', "¿Está usted seguro de modificar la lista de productos?",
                                        QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if respuesta == QMessageBox.Yes:
            filaSeleccionada = self.listaProductos.selectedItems()
            if filaSeleccionada:
            ...
            self.listaProductos.setSelectionBehavior(
                QAbstractItemView.SelectItems)
            # self.btnGuardar.clicked.connect(self.guardar)
            # lee valor de la primera columna, el identificador de la fila que modificara
            idProducto = str(self.listaProductos.item(indice, 0).text())
    ...
    def guardar(self):
        ...
        fila = self.listaProductos.currentRow()
        # lee valor de la primera columna, el identificador de la fila que modificara
        idProducto = str(self.listaProductos.item(fila, 0).text())
        if columna == -1:
            ...
            QMessageBox.critical(
                self, 'Error', "No puede modificar el campo codigo, intente nuevamenta",	QMessageBox.Ok)
            ...
            # el valor que le dio al campo que modificará
            valor = self.listaProductos.currentItem().text()
            # print("VALOR QUE SE LE DIO AL CAMPO A MODIFICAR " + valor)
            cursor = conn.cursor()
            cursor.execute("INSERT INTO precio_producto(idProducto,fecha,precioCosto,precioVenta) VALUES (%s,%s,%s,%s)",
                           (idProducto, fecha, precioCosto, precioVenta))
            consulta = cursor.fetchone()
            conn.commit()
            conn.close()
        elif columna == 7:
            ...
            if consulta:
                ...
                # print(idCategoria)
                sql = "UPDATE productos SET idCategoriaProducto = " + \
                    str(idCategoria) + " WHERE idProducto = " + str(idProducto)
                # print(sql)
                cursor.execute(sql)
                conn.commit()
                conn.close()
                ...
                QMessageBox.critical(
                    self, 'Error', "La Categoría de productos indicada no está en la base de datos. Indique la categoria correcta", QMessageBox.Ok)
        elif columna == 8:
            # print("id columna: " + str(columna))
            ...
            if consulta:
                ...
                # print(idProveedor)
                sql = "UPDATE producto_proveedor SET idProveedor = " + \
                    str(idProveedor) + " WHERE idProducto = " + str(idProducto)
                # print(sql)
                cursor.execute(sql)
                conn.commit()
                conn.close()
                ...
                QMessageBox.critical(
                    self, 'Error', "El proveedor indicado no está en la base de datos. Indique un proveedor válido", QMessageBox.Ok)
        else:
            # print("id columna: " + str(columna))
            # la fila que modificará, solo una a la vez
            fila = self.listaProductos.currentRow()
            # el id de la fila que modificara
            # lee valor de la primera columna, el identificadors
            id = self.listaProductos.item(fila, 0).text()
            id = str(id)
            # el valor que le dio al campo que modificará
            valor = self.listaProductos.currentItem().text()
            item = ['idProducto', 'nombreProducto', 'stock', 'stockCritico',
                    'stockMaximo', 'precioCosto', 'precioVenta', 'idCategoriaProducto']
            campo = item[columna]
            conn = pymysql.connect(
                host='localhost', user='root', password='JPTapia123', db='mydb')
            cursor = conn.cursor()
            sql = ("UPDATE productos SET " + campo + " = '" +
                   valor + "' WHERE idProducto = " + id)
            # print(sql)
            cursor.execute(sql)
            conn.commit()
            conn.close()
