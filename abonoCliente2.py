...
class Window(QMainWindow):
    ...
    def aceptar(self):
        ...
        if(totalAbono and efectivo):
            #totalAbono = int(totalAbono)
            #efectivo = int(efectivo)
            deudaTotal = int(self.lcdSaldoAdeudado.value())
            vuelto = int(efectivo) - int(totalAbono)
            self.lcdVuelto.setValue(vuelto)
            self.btnFin.setEnabled(True)
        else:
            QMessageBox.information(self, 'Error', "Debe ingresar monto a abonar y cantidad de efectivo con que pagará",QMessageBox.Ok)

    def finalizar(self):
        QMessageBox.warning(self, 'Advertencia', "¿Está usted seguro de abonar dinero a su cuenta?",QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
